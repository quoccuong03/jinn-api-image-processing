...
func readParams(query url.Values,data *Data) ImageOptions {
	params := make(map[string]interface{})

	//khi co ton tai post image thi url se bang file_name
	//neu query khong co height and witdh thi se gank dc gank chieu cao va chieu rong goc
	if len(query["width"])==0&&len(query["height"])==0{
		query.Set("width",data.OriginalWidth)
		query.Set("height",data.OriginalHeight)

	}

	if query.Get("watermark")=="true"{
		data.Watermark=true
	}else if query.Get("watermark")=="false"{
